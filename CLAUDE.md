...
- **Always ask before assuming.** If a requirement is unclear, ambiguous, or could be interpreted multiple ways, stop and ask before writing code. Even for simple bugs, ask for clarification rather than assuming intent.
...
- **Use `grill-me` for non-trivial tasks.** For any new feature, architectural change, or ambiguous requirement, invoke the `grill-me` skill before writing any code.
- **Update the README for user-facing changes.** After any user-facing feature is added, changed, or removed, update `README.md` to reflect it. Internal refactors (renaming hooks, restructuring files) do not require README updates.
...
- **`grill-me`** — for any task involving design decisions, new features, architectural changes, or ambiguous requirements. Invoke before writing any code.
...
4. Update `README.md` with the new tool's description.
